...
<div class="table-header">
    ...
    <a href="/faltas/adicionar" class="btn-nav-primary" style="background: var(--danger);"><i class="fas fa-plus"></i>
        Registrar Falta</a>
</div>
% if resumo:
<div style="margin-bottom: 2rem;">
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        ...
        <div class="card-box"
            style="padding: 1.5rem; display: flex; align-items: center; justify-content: space-between; background: {{cor_fundo}}; border: 1px solid {{cor_texto}};">
            <div>
                <h4 style="color: var(--secondary); font-weight: 800;">{{materia}}</h4><span
                    style="font-size: 0.85rem; color: var(--text-gray);">Total</span>
            </div>
            <div style="text-align: right;"><span
                    style="font-size: 1.8rem; font-weight: 800; color: {{cor_texto}};">{{qtd}}</span></div>
        </div>
        % end
    </div>
</div>
...
<table class="clean-table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><span
                    style="background: #fff5f5; color: var(--danger); padding: 4px 10px; border-radius: 20px; font-weight: 700; font-size: 0.85rem;">{{f.quantidade}}</span>
            </td>
            <td style="text-align: right;">
                <a href="/faltas/editar/{{f.id}}" class="btn-icon btn-edit"><i class="fas fa-pen"></i></a>
                <form action="/faltas/excluir/{{f.id}}" method="post" style="display:inline;"
