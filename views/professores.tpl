...
% else:
    <table class="clean-table">
        <thead><tr><th>Nome</th><th>Matéria</th><th>Email</th><th>Contato ou Sala</th><th style="text-align: right;">Ações</th></tr></thead>
        <tbody>
            % for p in professores:
            <tr>
                <td style="font-weight: 700; color: var(--secondary);">{{p.nome}}</td>
                <td><span style="background: #fff0f6; color: var(--primary); padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 700;">{{p.materia}}</span></td>
                <td>{{p.email}}</td>
                <td>{{p.contato}}</td>
                <td style="text-align: right;">
                    <a href="/professores/editar/{{p.id}}" class="btn-icon btn-edit"><i class="fas fa-pen"></i></a>
                    <form action="/professores/excluir/{{p.id}}" method="post" style="display:inline;" onsubmit="return confirm('Excluir?');">
